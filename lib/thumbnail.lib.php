...
function get_app_thumbnail($bo_table, $wr_id, $thumb_width, $thumb_height, $bf_no='0', $is_create=false, $is_crop=false, $crop_mode='center', $is_sharpen=false, $um_value='80/0.5/3')
{
    global $g5, $config;
    $filename = $alt = "";
    $edt = false;
    ...
    $sql = " select bf_file, bf_content from {$g5['board_file_table']}
                where bo_table = '$bo_table' and wr_id = '$wr_id' and bf_type between '1' and '3' and bf_no='$bf_no' order by bf_no limit 0, 1 ";
    $row = sql_fetch($sql);

    if($row['bf_file']) {
        $filename = $row['bf_file'];
        $filepath = G5_DATA_PATH."/".$bo_table;
        $alt = get_text($row['bf_content']);
    } else {
        $write_table = $g5['write_prefix'].$bo_table;
        $sql = " select wr_content from $write_table where wr_id = '$wr_id' ";
    ...
    if($tname) {
        if($edt) {
            ...
            $ori = G5_DATA_URL.'/'.$bo_table.'/'.$filename;
            $src = G5_DATA_URL.'/'.$bo_table.'/'.$tname;
        }
    } else {
        return false;
    }
