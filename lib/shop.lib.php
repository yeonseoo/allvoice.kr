...
function get_order_info($od_id)
{
    ...
    $sql = " select SUM(IF(io_type = 1, (io_price * ct_qty), ((ct_price + io_price) * ct_qty))) as price,
                ...
                from {$g5['g5_shop_cart_table']}
                where od_id = '$od_id'
                  and ct_status IN ( '진행중', '거래합의중', '의뢰완료', '작업진행중', '작업완료' ) ";
    $sum = sql_fetch($sql);
    ...
    $sql = " select SUM(IF(io_type = 1, (io_price * ct_qty), ((ct_price + io_price) * ct_qty))) as price
                from {$g5['g5_shop_cart_table']}
                where od_id = '$od_id'
                  and ct_status IN ( '취소', '의뢰취소', '품절' ) ";
    $sum = sql_fetch($sql);
    $cancel_price = $sum['price'];

	/* 미수금 관련 수정 */
	$q = "select * from g5_order_tax where od_id='".$od_id."' ";
	$chk_rows = sql_fetch($q);

	if ($chk_rows["tax_cash_select"] != "신청안함") {
		$cart_price = $cart_price + ( $cart_price * 0.1 );
		$cart_price = ceil($cart_price);
	}
	/* 미수금 관련 수정 */
...
// 사용후기의 확인된 건수를 상품테이블에 저장합니다.
function update_use_cnt2($it_id)
{
    global $g5;
    $row = sql_fetch(" select count(*) as cnt from {$g5['g5_shop_item_use_table']} where it_id = '{$it_id}' and is_confirm = 1 ");
    return sql_query(" update {$g5['member_table']} set it_use_cnt = '{$row['cnt']}' where mb_no = '{$it_id}' ");
}


// 사용후기의 선호도(별) 평균을 상품테이블에 저장합니다.
function update_use_avg2($it_id)
{
    global $g5;
    $row = sql_fetch(" select count(*) as cnt, sum(is_score) as total from {$g5['g5_shop_item_use_table']} where it_id = '{$it_id}' and is_confirm = 1 ");
    $average = ($row['total'] && $row['cnt']) ? $row['total'] / $row['cnt'] : 0;
    return sql_query(" update {$g5['member_table']} set it_use_avg = '$average' where mb_no = '{$it_id}' ");
}
