...
function get_paging_bak($write_pages, $cur_page, $total_page, $url, $add="")
{
...
//echo "start_page = ".$start_page;
    ...
    if ($start_page > 1) $str .= '<a href="'.$url.($start_page-1).$add.'" ><img src="'.G5_URL.'/theme/basic/img/btn_listPrev.png" /></a>'.PHP_EOL;
    ...
    if ($total_page > $end_page) $str .= '<a href="'.$url.($end_page+1).$add.'" ><img src="'.G5_URL.'/theme/basic/img/btn_listNext.png" /></a>'.PHP_EOL;
...
//echo "str = ".$str;exit;
    if ($str)
        return "<div class=\"ttrxPaging\">".$str."</div>";
    else
        return "";
}
...
// 비즈뿌리오 SMS/LMS 전송 함수
// $wr_reply 발신번호, $wr_subject 문자 제목, $wr_message 문자 내용, $list 수신자 전화번호 배열, $wr_booking 예약발송 여부, 즉시발송이면 '' 예약이면 예약 일시 2020-01-01 12:12:00, $mb_id sms/lms 발신자 아이디, $admin_no 가맹점 mb_no
function sms_send ( $wr_reply=SMS_REPLY_TEL , $wr_subject='', $wr_message, $list, $wr_booking='', $mb_id='', $admin_no ) {
	global $g5;

	// CINFO 에 비즈뿌리오 하위계정아이디 입력
	$cacao_arr = sql_fetch("SELECT cacao_id, mb_level FROM ".$g5['member_table']." WHERE mb_no='".$admin_no."'");
	...
	if ( $wr_booking == "" ) {
		...
		$wr_booking = "0000-00-00 00:00:00";
	}
	else $booking = $wr_booking;

	$bytes = mb_strlen( $wr_message, 'euc-kr' );
	if ( $bytes > 90 ) $lms = 5;
	else $lms = 0;

	$auto = $mb_id ? "수동" : "자동";

	// hs_flag 성공/실패, MSG_TYPE sms 0, lms 5
	for ($i = 0; $i < count($list); $i++) {
		if ( $list[$i] ) {

			$cmid = date("YmdHis").sprintf("%04d",($i+1));

			if ( $cacao_arr['mb_level'] == "13" )
				$sql = "INSERT INTO BIZ_MSG ( MSG_TYPE, CMID, REQUEST_TIME, SEND_TIME, DEST_PHONE, SEND_PHONE, MSG_BODY) VALUES (".$lms.", '".$cmid."', NOW(), '".$booking."', '".str_replace("-","",$list[$i])."', '".$wr_reply."', '".addslashes($wr_message)."') ";
			else
				$sql = "INSERT INTO BIZ_MSG ( MSG_TYPE, CMID, REQUEST_TIME, SEND_TIME, DEST_PHONE, SEND_PHONE, MSG_BODY, CINFO) VALUES (".$lms.", '".$cmid."', NOW(), '".$booking."', '".str_replace("-","",$list[$i])."', '".$wr_reply."', '".addslashes($wr_message)."','".$cacao_arr['cacao_id']."') ";
			//echo $sql."<br>";
			sql_query($sql);
			sql_query("insert into {$g5['sms5_history_table']} set wr_no='$wr_no', wr_renum='{$admin_no}', mb_id='{$mb_id}', hs_hp='{$list[$i]}', hs_datetime='".G5_TIME_YMDHIS."', hs_flag='', hs_code='', hs_memo='".addslashes($wr_message)."', hs_log='".$cmid."'", false);
		}
	}

	sql_query("insert into {$g5['sms5_write_table']} set wr_no='$wr_no', wr_renum='{$admin_no}', wr_reply='$wr_reply', wr_subject='".addslashes($wr_subject)."', wr_message='".addslashes($wr_message)."', wr_success='', wr_failure='', wr_memo='".$auto."', wr_booking='$wr_booking', wr_total='$wr_total', wr_datetime='".G5_TIME_YMDHIS."'");
}

/*
각 주문상태별 비즈뿌리오 SMS 전송
$od_id => 주문번호
$od_status => 1:주문완료, 2:상품준비중, 3:배송중, 4:배송완료, 5:취소신청, 6:교환신청, 7:반품신청, 8:교환완료, 9:반품완료, 10:취소완료
예) sms_auto_send('1563-190717174637299889', 1);
*/
function sms_auto_send($od_id, $od_status) {
	global $g5;

	$sql = "SELECT * FROM ".$g5['g5_shop_order_table']." WHERE od_id='".$od_id."'";
	$row = sql_fetch($sql);
//print_r($stat);print_r($row);
	$list = NULL;
	$list[] = $row['od_hp'];

	$qry = "SELECT * FROM sms5_form WHERE fg_no='".$od_status."' AND admin_no='".$row['it_member_id']."'";
	$stat = sql_fetch($qry);

	$message = $stat['fo_content'];
	$message = preg_replace("/{이름}/", $row['od_name'], $message);
	$message = preg_replace("/{주문번호}/", $row['od_id'], $message);
	$message = preg_replace("/{결제금액}/", number_format($row['od_cart_price'] + $row['od_send_cost'] + $row['od_send_cost2']), $message);
	$message = preg_replace("/{취소금액}/", number_format($row['od_cancel_price']), $message);
	$message = preg_replace("/{택배회사}/", $row['od_delivery_company'], $message);
	$message = preg_replace("/{운송장번호}/", $row['od_invoice'], $message);
	$message = preg_replace("/{상품URL}/", G5_SHOP_URL."/item.php?it_id=".$row['it_id'], $message);
//echo $message;
	sms_send ( SMS_REPLY_TEL , $stat['fo_name'], $message, $list, '', '', $row['it_member_id'] );
}
