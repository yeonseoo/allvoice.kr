...
//===============================================================
// 성우회원의 경우 관리자의 인증이 필요함
$mb_level = ( $mb_gubun == 3 ) ? 1 : $config['cf_register_level'];
if ($w == '') {
...
$reg_url = ( $member['mb_gubun'] == "3" ) ? "register_form2.php" : "register_form.php";

if ($msg)
    echo '<script>alert(\''.$msg.'\');</script>';

if ($w == '') {
	// 성우회원 가입시 admin 핸드폰번호에 sms를 보냄
	if ( $mb_gubun == 3 ) {
		...
		$cmt = "성우회원 가입 신청이 도착했습니다. -올보이스-";
		$sms_content = iconv_euckr( $cmt );

		$biz_sql = "INSERT INTO BIZ_MSG (MSG_TYPE, CMID, REQUEST_TIME, SEND_TIME, DEST_PHONE, SEND_PHONE,  MSG_BODY, TEMPLATE_CODE, SENDER_KEY, NATION_CODE, ATTACHED_FILE, RE_TYPE, RE_BODY) VALUES (6, '".date("YmdHis",time()+1)."', NOW(), NOW(), '".$recv_number."', '".$send_number."', '".$cmt."', 'bizp_2019091010342116788527056', '".SENDER_KEY."', '82', 'kakao_button.json', 'SMS', '".$cmt."')";
		//echo $biz_sql;
		sql_query( $biz_sql );

        // admin 계정 연락처만 보낼수 있어서 임시로 작업
        $recv_number2 = "01024452055";
        $biz_sql2 = "INSERT INTO BIZ_MSG (MSG_TYPE, CMID, REQUEST_TIME, SEND_TIME, DEST_PHONE, SEND_PHONE,  MSG_BODY, TEMPLATE_CODE, SENDER_KEY, NATION_CODE, ATTACHED_FILE, RE_TYPE, RE_BODY) VALUES (6, '".date("YmdHis",time()+2)."', NOW(), NOW(), '".$recv_number2."', '".$send_number."', '".$cmt."', 'bizp_2019091010342116788527056', '".SENDER_KEY."', '82', 'kakao_button.json', 'SMS', '".$cmt."')";
        sql_query( $biz_sql2 );
...
//echo "insert into {$g5['sms5_history_table']} set wr_no='$wr_no', wr_renum=0, bg_no='0', mb_id='{$row['mb_id']}', bk_no='0', hs_name='', hs_hp='{$recv_number}', hs_datetime='".G5_TIME_YMDHIS."', hs_flag='$hs_flag', hs_code='$hs_code', hs_memo='".addslashes($hs_memo)."', hs_log='".addslashes($log)."'";
    ...
    if ($old_email != $mb_email && $config['cf_use_email_certify']) {
        ...
        <title>회원정보수정</title>
        <body>
        <form name="fregisterupdate" method="post" action="'.G5_HTTP_BBS_URL.'/'.$reg_url.'">
