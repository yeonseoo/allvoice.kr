...
// 거래제한
$mb_order_block_yes = $mb['mb_order_block_yn'] ? 'checked="checked"' : '';
$mb_order_block_no = !$mb['mb_order_block_yn'] ? 'checked="checked"' : '';
...
<div class="tbl_frm01 tbl_wrap">
    ...
    <tr>
        ...
        <td>
			...
			<select id="mb_gubun" name="mb_gubun">
				<option value="1" <?php echo $mb['mb_gubun'] == "1" ? "selected" : ""; ?>>일반회원</option>
				<option value="2" <?php echo $mb['mb_gubun'] == "2" ? "selected" : ""; ?>>기업회원</option>
				<option value="3" <?php echo $mb['mb_gubun'] == "3" ? "selected" : ""; ?>>협회성우</option>
                <option value="4" <?php echo $mb['mb_gubun'] == "4" ? "selected" : ""; ?>>일반성우</option>
			</select>
		</td>
        <th scope="row">포인트</th>
    ...
    <tr>
        ...
        <th scope="row">안심번호(성우)</th>
        <td>
            <?php
            // 안심번호 설정 확인
            if (true == isset($mb['mb_safe_no'])) {
                $safe_no_format = $mb['mb_safe_no'];

                if (12 == strlen($mb['mb_safe_no'])) {
                    $safe_no_format = preg_replace("/([0-9]{4})([0-9]{4})([0-9]{4})/", "$1-$2-$3", $mb['mb_safe_no']);
                }
                echo $safe_no_format;
            }
            else { ?>
                <!-- <a href="./boardgroupmember_form.php?mb_id=<?php echo $mb['mb_id'] ?>" class="btn_frmline">접근가능그룹보기</a>-->
                <button type="button" class="btn_frmline" onclick="createSafeNo()">안심번호 생성</button>
            <?php
            }
            ?>
        </td>
    ...
    <tr>
        <th scope="row">거래제한</th>
        <td>
            <input type="radio" name="mb_order_block" value="1" id="mb_order_block_yes" <?php echo $mb_order_block_yes; ?>>
            <label for="mb_order_block_yes">예</label>
            <input type="radio" name="mb_order_block" value="0" id="mb_order_block_no" <?php echo $mb_order_block_no; ?>>
            <label for="mb_order_block_no">아니오</label>
        </td>
        <!--
        <th scope="row">거래제한 날짜</th>
        <td>
        -->
    </tr>
    <tr>
        <th scope="row"><label for="mb_order_block_comment">거래제한 사유</label></th>
        <td colspan="3"><textarea name="mb_order_block_comment" id="mb_order_block_comment"><?php echo $mb['mb_order_block_comment'] ?></textarea></td>
    </tr>
    <tr>
        <th scope="row">주소</th>
...
this.form.mb_intercept_date.value=this.form.mb_intercept_date.defaultValue; }">
	...
	<tr>
        <th scope="row"><label for="mb_title">프로필제목</label></th>
        <td colspan="3"><input type="text" name="mb_title" id="mb_title" value="<?php echo $mb['mb_title'] ?>" class="frm_input" style="width:100%;"></td>
    </tr>
    <tr>
    ...
    <tr>
        <th scope="row"><label for="mb_memo">주요작품 및 클라이언트</label></th>
	...
	?>
    <tr>
        <th scope="row"><label for="mb_<?php echo $i ?>"><?php echo $extra_name ?></label></th>
	...
	?>
    <tr>
        <th scope="row"><label for="mb_network_from">방송국</label></th>
        <td colspan="3">
            <select id="mb_network_from" name="mb_network_from">
                <option value="" <?php echo $mb['mb_network_from'] == "" ? "selected" : ""; ?>></option>
                <option value="EBS" <?php echo $mb['mb_network_from'] == "EBS" ? "selected" : ""; ?>>EBS</option>
                <option value="KBS" <?php echo $mb['mb_network_from'] == "KBS" ? "selected" : ""; ?>>KBS</option>
                <option value="대원" <?php echo $mb['mb_network_from'] == "대원" ? "selected" : ""; ?>>대원</option>
                <option value="대교" <?php echo $mb['mb_network_from'] == "대교" ? "selected" : ""; ?>>대교</option>
                <option value="CJ&M(투니버스)" <?php echo $mb['mb_network_from'] == "CJ&M(투니버스)" ? "selected" : ""; ?>>CJ&M(투니버스)</option>
            </select>
        </td>
    </tr>
    <tr>
        <th scope="row"><label for="mb_network_year">방송국 입사년도</label></th>
        <td colspan="3">
            <input type="text" name="mb_network_year" value="<?php echo $mb['mb_network_year'] ?>" id="mb_network_year" class="frm_input" size="30" maxlength="255">
        </td>
    </tr>
...
function createSafeNo() {

    $.ajax({
        type: "POST",
        url: g5_admin_url+"/member_safe_number_update.php",
        dataType: "json",
        data: { "mb_id" : $('#mb_id').val(), "mb_hp" : $('#mb_hp').val()},
