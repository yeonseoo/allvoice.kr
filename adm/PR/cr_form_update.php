...
$sql_common = " pr_id = '{$_POST['pr_id']}', 
				cr_subject = '{$_POST['cr_subject']}', 
				cr_info = '{$_POST['cr_info']}' ";

if($w == ""){

	sql_query(" insert into AV_CR set cr_regdate = '".G5_TIME_YMDHIS."', {$sql_common} ");
	$cr_id = sql_insert_id();

}
else if ($w == "u")
{
    $sql = " update AV_CR set $sql_common where cr_id = '$cr_id' ";
    sql_query($sql);
}
else if ($w == "d")
{

    $sql = " delete from AV_CR where cr_id = '$cr_id' ";
    sql_query($sql);
...
alert("처리되었습니다.","./cr_form.php?w=u&amp;cr_id=$cr_id&pr_id=$pr_id");
?>
