...
if($w == ""){
	...
	sql_query(" insert into AV_CN set cn_regdate = '".G5_TIME_YMDHIS."', {$sql_common} ");
	$cn_id = sql_insert_id();

}
else if ($w == "u")
{
    $sql = " update AV_CN set $sql_common where cn_id = '$cn_id' ";
    sql_query($sql);
}
else if ($w == "d")
{

    $sql = " delete from AV_CN where cn_id = '$cn_id' ";
    sql_query($sql);
...
alert("처리되었습니다.","./cn_form.php?w=u&amp;cr_id=$cr_id&cn_id=$cn_id");
?>
