...
if( !in_array($sel_field, array('od_id', 'd.mb_id', 'd.mb_name', 'd.mb_tel', 'd.mb_hp', 'od_b_name', 'od_b_tel', 'od_b_hp', 'od_deposit_name', 'od_invoice')) ){   //검색할 필드 대상이 아니면 값을 제거
    $sel_field = '';
}
...
$_REQUEST['it_gubun'] = $_REQUEST['it_gubun'] ? $_REQUEST['it_gubun'] : "a";
if ( $_REQUEST['it_gubun'] != "a" ) {
	if ( $_REQUEST['it_gubun'] == "3" ) $where[] = " od_id IN ( SELECT a.od_id FROM ".$g5['g5_shop_cart_table']." AS a JOIN ".$g5['g5_shop_item_table']." AS b ON a.it_id=b.it_id WHERE b.it_gubun='3' ) ";
	else $where[] = " od_id IN ( SELECT a.od_id FROM ".$g5['g5_shop_cart_table']." AS a JOIN ".$g5['g5_shop_item_table']." AS b ON a.it_id=b.it_id WHERE b.it_gubun IN ('1','2') ) ";
}
...
$sql_common = " from {$g5['g5_shop_order_table']} a JOIN {$g5['g5_shop_cart_table']} b ON b.od_id=a.od_id JOIN {$g5['g5_shop_item_table']} c ON c.it_id=b.it_id AND c.it_origin=b.mb_id JOIN {$g5['member_table']} d ON c.it_maker=d.mb_id $sql_search ";
...
$sql  = " select a.*,d.mb_id,d.mb_name,d.mb_nick,d.mb_email,
...
<select name="it_gubun" id="it_gubun">
	<option value="a" <?php echo $_REQUEST['it_gubun'] == "a" ? "selected" : ""; ?>>작업의뢰/합의주문</option>
	<option value="1" <?php echo $_REQUEST['it_gubun'] == "1" ? "selected" : ""; ?>>작업의뢰</option>
	<option value="3" <?php echo $_REQUEST['it_gubun'] == "3" ? "selected" : ""; ?>>합의주문</option>
</select>
<select name="sel_field" id="sel_field">
    <option value="od_id" <?php echo get_selected($sel_field, 'od_id'); ?>>주문번호</option>
    <option value="d.mb_id" <?php echo get_selected($sel_field, 'd.mb_id'); ?>>회원 ID</option>
    <option value="d.mb_name" <?php echo get_selected($sel_field, 'd.mb_name'); ?>>주문자</option>
    <option value="d.mb_tel" <?php echo get_selected($sel_field, 'd.mb_tel'); ?>>주문자전화</option>
    <option value="d.mb_hp" <?php echo get_selected($sel_field, 'd.mb_hp'); ?>>주문자핸드폰</option>
...
<div>
	...
	<input type="radio" name="od_status" value="진행중" id="od_status_income" <?php echo get_checked($od_status, '진행중'); ?>>
    <label for="od_status_income">진행중</label>
	<input type="radio" name="od_status" value="작업진행중" id="od_status_income" <?php echo get_checked($od_status, '작업진행중'); ?>>
    <label for="od_status_income">작업진행중</label>
	<input type="radio" name="od_status" value="의뢰완료" id="od_status_income" <?php echo get_checked($od_status, '의뢰완료'); ?>>
    <label for="od_status_income">의뢰완료</label>
	<input type="radio" name="od_status" value="거래합의중" id="od_status_income" <?php echo get_checked($od_status, '거래합의중'); ?>>
    <label for="od_status_income">거래합의중</label>
	<input type="radio" name="od_status" value="작업진행중" id="od_status_income" <?php echo get_checked($od_status, '작업진행중'); ?>>
    <label for="od_status_income">작업진행중</label>
	<input type="radio" name="od_status" value="작업완료" id="od_status_income" <?php echo get_checked($od_status, '작업완료'); ?>>
    <label for="od_status_income">작업완료</label>
	<input type="radio" name="od_status" value="의뢰취소" id="od_status_income" <?php echo get_checked($od_status, '의뢰취소'); ?>>
    <label for="od_status_income">의뢰취소</label>
...
<div class="tbl_head01 tbl_wrap">
    ...
    <tr>
        ...
        <th scope="col" id="th_recvr">성우회원</th>
        ...
        <th scope="col" rowspan="3">세금처리여부</th>
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        $mb_nick = get_sideview($row['it_maker'], get_text($row['mb_name']), $row['mb_email'], '');

        $od_cnt = 0;
        if ($row['it_maker'])
        {
            $sql2 = " select count(*) as cnt from {$g5['g5_shop_order_table']} a JOIN {$g5['g5_shop_cart_table']} b ON b.od_id=a.od_id JOIN {$g5['g5_shop_item_table']} c ON c.it_id=b.it_id AND c.it_origin=b.mb_id where it_maker = '{$row['it_maker']}' ";
            $row2 = sql_fetch($sql2);
            $od_cnt = $row2['cnt'];
        }
        ...
        $q_ = "select tax_cash_select from g5_order_tax where od_id='".$row['od_id']."' ";
        $tax_arr = sql_fetch($q_);
    ?>
    <tr class="orderlist<?php echo ' '.$bg; ?>">
        ...
        <td rowspan="3" class="td_num_right"><?php echo $tax_arr['tax_cash_select'] == "" ? "-" : $tax_arr['tax_cash_select']; ?></td>
    ...
    <tr class="<?php echo $bg; ?>">
        <td headers="th_odrid">
            <?php if ($row['mb_id']) { ?>
            <a href="<?php echo $_SERVER['SCRIPT_NAME']; ?>?sort1=<?php echo $sort1; ?>&amp;sort2=<?php echo $sort2; ?>&amp;sel_field=d.mb_id&amp;search=<?php echo $row['mb_id']; ?>"><?php echo $row['mb_id']; ?></a>
            <?php } else { ?>
            비회원
            <?php } ?>
        </td>
