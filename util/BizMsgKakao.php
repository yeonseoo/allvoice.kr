...
abstract class biz_msg_type
{
    ...
    const biz_msg_code_11 = 11;         // 메세지 도착
    const biz_msg_code_12 = 12;         // 메세지 알림 성우 > 회원
    const biz_msg_code_13 = 13;         // 메세지 알림 회원 > 성우
}
...
class BizMsgKakao
{
    ...
    {
        $dateNow = date_create('now')->format('YmdHis');
        $cmid = $this->getCmid();
    ...
    public function insertInboxMessageBizMsgToDB($destPhone, $sendPhone, $msgType, $nick, $message)
    {
        $dateNow = date_create('now')->format('YmdHis');
        $cmid = $this->getCmid();
        $templeteCode = $this->getTempleteCode($msgType);
        ...
        $bizMsg = sprintf($bizMsg, $nick, $message);
        ...
        $bizSql = "
            INSERT INTO BIZ_MSG
             SET    MSG_TYPE        = 6, 
                    ...
                    SENDER_KEY      = '" . SENDER_KEY . "', 
                    NATION_CODE     = '82', 
                    ATTACHED_FILE   = 'kakao_inbox_button.json', 
                    RE_TYPE         = 'SMS', 
                    RE_BODY         = '" . $smsMsgBody . "' 
        ";
    ...
    private function getTempleteCode($msgType)
    {
        ...
        switch ($msgType) {
            ...
            case biz_msg_type::biz_msg_code_10:
                ...
                break;
            case biz_msg_type::biz_msg_code_11:
                $templeteCode = "bizp_2020060114514126181155074";     // 1:1 메세지 도착
                break;
            case biz_msg_type::biz_msg_code_12:
                $templeteCode = "bizp_2020100614390500676226785";     // 메세지 알림 성우 > 회원
                break;
            case biz_msg_type::biz_msg_code_13:
                $templeteCode = "bizp_2020100614403832253429733";     // 메세지 알림 회원 > 성우
                break;
        }
    ...
    private function getMsgBody($msgType)
    {
        ...
        switch ($msgType) {
            ...
            case biz_msg_type::biz_msg_code_11:
                // 결제완료>To.일반(200413 수정)
                $msgBody = '[올보이스]\r\n\r\n';
                $msgBody = $msgBody . '1:1 문의에 새로운 메시지가 도착하였습니다.\r\n';
                $msgBody = $msgBody . '올보이스에 로그인 한 뒤 메시지를 확인해주시길 바랍니다.';
                break;
            case biz_msg_type::biz_msg_code_12:
                $msgBody = '%s 성우님이 회원님께 녹음 견적에 대한 메세지를 보냈습니다.\r\n\r\n';
                $msgBody = $msgBody . '메세지 내용:\r\n';
                $msgBody = $msgBody . '%s\r\n\r\n';
                $msgBody = $msgBody . '올보이스 메세지를 확인해보세요.';
                break;
            case biz_msg_type::biz_msg_code_13:
                $msgBody = '%s 회원님이 성우님께 녹음 견적에 대한 메세지를 보냈습니다.\r\n\r\n';
                $msgBody = $msgBody . '메세지 내용:\r\n';
                $msgBody = $msgBody . '%s\r\n\r\n';
                $msgBody = $msgBody . '올보이스 메세지를 확인해보세요.';
                break;
    ...
    private function getSmsMsgBody($msgType)
    {
        ...
        switch ($msgType) {
            ...
            case biz_msg_type::biz_msg_code_10:
                ...
                break;
            case biz_msg_type::biz_msg_code_11:
                $smsMsgBody = "1:1메세지가 도착하였습니다. -올보이스-";
                break;
            case biz_msg_type::biz_msg_code_12:
                $smsMsgBody = "1:1 성우님의 메세지가 도착하였습니다. -올보이스-";
                break;
            case biz_msg_type::biz_msg_code_13:
                $smsMsgBody = "1:1 회원님의 메세지가 도착하였습니다. -올보이스-";
                break;
        }
