...
if(isset($data['pp_id']) && $data['pp_id']) {
	...
	$order_action_url = G5_HTTPS_SHOP_URL.'/voiceMypageOrderDetail_update.php';
    $page_return_url  = G5_SHOP_URL.'/voiceMypageOrderDetail.php?it_id='.$data['it_id'].'&od_id='.$data['od_id'].'&it_gubun='.$data['it_gubun'];
    //if($_SESSION['ss_direct'])
        //$page_return_url .= '?it_id='.$data['it_id'].'&od_id='.$data['od_id'].'&it_gubun='.$data['it_gubun'];
//it_id=1556520011&od_id=2019043011281836&it_gubun=1
    // 장바구니가 비어있는가?
...
if($_REQUEST['P_STATUS'] != '00') {
    ...
    $return = curl_exec($ch);
//print_r($_REQUEST);print_r($post_data);print_r($return);exit;
    if(!$return)
