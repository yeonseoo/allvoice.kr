...
if (!$is_member) {
    alert_close("응원메세지는 회원만 작성이 가능합니다.");
}
...
if($is_mobile_shop)
	$url = "./voiceDetail.php?it_id=".$it_id."#voiceDetailSection2";
...
else if ($w == "d")
{
    if (!$is_admin)
    {
        ...
        $row = sql_fetch($sql);
        if (!$row)
            alert("자신의 응원메세지만 삭제하실 수 있습니다.");

        if ($row['iq_answer'])
            alert("답변이 있는 응원메세지는 삭제하실 수 없습니다.");
    }
