...
if (G5_IS_MOBILE) {
    include_once(G5_MSHOP_PATH . '/voiceList_n.php');
    return;
}
...
require_once(G5_THEME_SHOP_PATH.'/shop.head_n.php');
...
$qry = "select count(*) as rows 
        from 
            (SELECT count(*) as cnt FROM g5_member_voice a JOIN g5_member b ON a.mb_id=b.mb_id where a.mv_cat = '" . $_REQUEST['cat'] . "' " . $sub_where . " AND b.mb_gubun = '4' group by a.mb_id) as c";
...
$qry = "SELECT a.*, b.mb_name, b.it_use_avg, c.cnt FROM " . $g5['member_voice'] . " a 
        ...
        WHERE a.mv_cat='" . $_REQUEST['cat'] . "' " . $sub_where . "  AND b.mb_gubun = '4' group by a.mb_id ORDER BY  " . $_REQUEST['orderby'] . " DESC, a.mb_id ASC, a.mv_no ASC LIMIT " . $from_record . ", " . $page_rows . " ";
//echo $qry;
$res = sql_query($qry);
            ...
            <div class="listInfo">
                <div class="listLocation">
                    <ul>
                        ...
                        <li>일반성우 > <?php echo $cat_dt['ca_name']; ?></li>
                        <!--li>카테고리명01</li-->
                    </ul>
                </div>
                <div class="listOrder">
