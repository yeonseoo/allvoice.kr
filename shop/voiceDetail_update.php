...
$cmt = "합의주문의뢰가 도착했습니다. -올보이스-";
$sms_content = iconv_euckr($cmt);

//$biz_sql = "INSERT INTO BIZ_MSG (MSG_TYPE, CMID, REQUEST_TIME, SEND_TIME, DEST_PHONE, SEND_PHONE,  MSG_BODY, TEMPLATE_CODE, SENDER_KEY, NATION_CODE, ATTACHED_FILE, RE_TYPE, RE_BODY) VALUES (6, '" . date("YmdHis") . "', NOW(), NOW(), '" . $recv_number . "', '" . $send_number . "', '" . $cmt . "', 'bizp_2019082914415822317275523', '" . SENDER_KEY . "', '82', 'kakao_button.json', 'SMS', '" . $cmt . "')";
//echo $biz_sql;
//sql_query($biz_sql);

// 22	bizp_2020041316321203031079912	합의주문>주문발생>To.성우(200413 수정)
$bizKakao = new BizMsgKakao;
$bizKakao->insertBizMsgToDB($recv_number, $send_number, biz_msg_type::biz_msg_code_7);
...
$recv_number = str_replace("-", "", $row['mb_hp']);
$send_number = $sms5['cf_phone'];
$cmt = "새로운 합의주문이 등록되었습니다. -올보이스-";
$sms_content = iconv_euckr($cmt);

$biz_sql = "INSERT INTO BIZ_MSG (MSG_TYPE, CMID, REQUEST_TIME, SEND_TIME, DEST_PHONE, SEND_PHONE,  MSG_BODY, TEMPLATE_CODE, SENDER_KEY, NATION_CODE, ATTACHED_FILE, RE_TYPE, RE_BODY) VALUES (6, '" . date("YmdHis", time() + 1) . "', NOW(), NOW(), '" . $recv_number . "', '" . $send_number . "', '" . $cmt . "', 'bizp_2019082914475522317334527', '" . SENDER_KEY . "', '82', 'kakao_button.json', 'SMS', '" . $cmt . "')";
//echo $biz_sql;
sql_query($biz_sql);

// admin 계정 연락처만 보낼수 있어서 임시로 작업
$recv_number2 = "01024452055";
$biz_sql2 = "INSERT INTO BIZ_MSG (MSG_TYPE, CMID, REQUEST_TIME, SEND_TIME, DEST_PHONE, SEND_PHONE,  MSG_BODY, TEMPLATE_CODE, SENDER_KEY, NATION_CODE, ATTACHED_FILE, RE_TYPE, RE_BODY) VALUES (6, '" . date("YmdHis", time() + 2) . "', NOW(), NOW(), '" . $recv_number2 . "', '" . $send_number . "', '" . $cmt . "', 'bizp_2019082914475522317334527', '" . SENDER_KEY . "', '82', 'kakao_button.json', 'SMS', '" . $cmt . "')";
sql_query($biz_sql2);
...
alert('합의주문의뢰가 완료되었습니다.', './voiceDetail.php?cat=' . $_REQUEST['cat'] . '&mb_id=' . $_REQUEST['mb_id']);
//goto_url('./voiceDetail.php?cat='.$_REQUEST['cat'].'&mb_id='.$_REQUEST['mb_id'], false);
?>